...
//Coco, bienvenue dans cette chimère de ton programme Essai_Capteurs et de l'exemple LoRaSender
int counter = 0;
...
//J'ai tout mis au début, les librairies LoRa et toutes ces bêtises (en priant que ça définisse tout au départ on va pas se mentir)
...
void loop() {
  Transfert_Info(200); //si on veut tout faire d'un coup, je pense qu'il faut enregistrer les infos sur la carte SD depuis cette loop
}
...
void setup() {
  ...
  Serial.println("Envoyeur LoRa");

  if (!LoRa.begin(868E6)) {
    Serial.println("LoRa n'a pas demarre");
    while (1);
  }
}
...
void loop() {
  ...
  // envoi des paquets de données: je mets la transfer_info dedans, ces infos sont envoyées par le module LoRa
  LoRa.beginPacket();
  Transfert_Info(200);
  LoRa.endPacket();

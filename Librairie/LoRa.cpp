...
int LoRa::begin(long frequency, int our_ss)
{
  _ss = our_ss;
  // setup pins
  pinMode(_ss, OUTPUT);
...
void LoRa::setSPI(SPIClass& spi)
{
  _spi = &spi;
}

void LoRa::setSPIFrequency(uint32_t frequency)
{
  _spiSettings = SPISettings(frequency, MSBFIRST, SPI_MODE0);
}

void LoRa::dumpRegisters(Stream& out)
{
  for (int i = 0; i < 128; i++) {
    out.print("0x");
    out.print(i, HEX);
